...
- **Server properties track a pinned Minecraft version.** `minecraft/server_properties.rs` mirrors the vanilla `server.properties` keys and defaults for the version named in the comment above `ServerProperties`. To move to a newer version, read the History section of the wiki page for every key added, removed, or defaulted differently since, mirror each change in the struct fields (alphabetical by serialized kebab-case name) and the `Default` impl, keep the deliberate deviations commented inline, then bump the version in that comment and in the `properties` section of `docs/src/reference/manifest.md`.
...
- **Tests live in a `tests.rs` next to the module's `mod.rs`** (`manifest/tests.rs`, `minecraft/tests.rs`), declared with `#[cfg(test)] mod tests;`. Never write an inline `mod tests` inside a source file.

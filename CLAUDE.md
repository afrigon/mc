...
- **`manifest/`** — `Manifest::from_kdl_str` decodes `mc.kdl` by hand over the `kdl` crate's document model (there is no serde adapter for KDL v2); each section implements `FromKdlNode`. `lock.rs` is the mod lockfile (`mc.lock`, also KDL). `document.rs` holds the format-preserving edits of the `mods` block used by `mc add`/`remove`/`update`. `presets.rs` generates manifests for the `vanilla`/`optimized`/`technical` presets. `tests.rs` covers decoding, editing, and preset output.
...
- **`utils/`** — `shell.rs` (Cargo-style status output), `errors.rs`, `archive.rs`, `product_descriptor.rs`, `kdl.rs` (typed getters, section validation, and quoted/indented node builders over the `kdl` crate), `verbosity.rs`.
